...
static MatrixXf Pixel2Matrix(unsigned short *pDepth);
...
int main(int argc, char* argv[])
{
	...
	//Add depth map=============================================================
	unsigned char *imgData = new unsigned char[IMG_WIDTH*IMG_HEIGHT*4];

	for(int i = 0; i < IMG_WIDTH*IMG_HEIGHT*4; ++i)
		imgData[i] = 255;

	img2d* imgDepth = new img2d(loadShader("./shader/vs_img2d.txt", "./shader/fs_img2d.txt"), imgData, IMG_WIDTH, IMG_HEIGHT);
	imgDepth->setColor(glm::vec3(1.0f, 1.0f, 1.0f));
	imgDepth->setUniformVec3Color("color_input");
	imgDepth->show(635, 475, IMG_WIDTH/2, IMG_HEIGHT/2);
	...
	while (glfwGetKey(window, GLFW_KEY_ESCAPE) != GLFW_PRESS
		...
		if (isStart && fp && FrameCount%speed == 0){
			//Read dvs file
	        size_t rsize = fread(pDepth, sizeof(unsigned short), IMG_WIDTH*IMG_HEIGHT, fp);
	        FrameCount = 0;
			...
			for(int i = 0; i < IMG_WIDTH*IMG_HEIGHT-1; ++i){
				imgData[4*i] = pDepth[IMG_WIDTH*IMG_HEIGHT-i]/10;
				imgData[4*i+1] = pDepth[IMG_WIDTH*IMG_HEIGHT-i]/10;
				imgData[4*i+2] = pDepth[IMG_WIDTH*IMG_HEIGHT-i]/10;
				imgData[4*i+3] = 255;
			}
			imgDepth->uploadImg(imgData, IMG_WIDTH, IMG_HEIGHT);

			//Update TSDF & camera pose
			pc_depthCam = Pixel2Matrix(pDepth);
