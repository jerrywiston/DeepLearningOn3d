...
/*====================================
Set model matrix in GLSL
====================================*/
bool visibleObj::setUniformMat4Model(const char* name)
{
	glUseProgram(this->program);
	GLint loc = glGetUniformLocation(this->program, name);
	if(loc == -1) return false;

	glUniformMatrix4fv(loc, 1, GL_FALSE, value_ptr(trans * rot * scal));
...
/*====================================
Set color vector in GLSL
====================================*/
bool visibleObj::setUniformVec3Color(const char* name)
{
	glUseProgram(this->program);
	GLint loc = glGetUniformLocation(this->program, name);
	if(loc == -1) return false;

	glUniform3fv(loc, 1, glm::value_ptr(this->color));
...
Set color
====================================*/
void visibleObj::setColor(vec3 color)
{
	this->color = color;
}
...
Access translation
==================================================*/
void visibleObj::setTrans(mat4 trans)
{
	this->trans = trans;
}

void visibleObj::setTrans(vec3 pos)
{
	this->trans = translate(mat4(), pos);
}

void visibleObj::setTrans(float x, float y, float z)
{
	this->trans = translate(mat4(), vec3(x, y, z));
}

mat4 visibleObj::getTrans()
{
	return this->trans;
}

vec3 visibleObj::getTransXYZ()
{
	return vec3(trans[3].x, trans[3].y, trans[3].z);
}
...
Access rotation
==================================================*/
void visibleObj::setRot(mat4 rot)
{
	this->rot = rot;
}

mat4 visibleObj::getRot()
{
	return this->rot;
}
...
Access scale
==================================================*/
void visibleObj::setScal(mat4 scal)
{
	this->scal = scal;
}

void visibleObj::setScal(vec3 scal)
{
	this->scal = scale(mat4(), scal);
}

void visibleObj::setScal(float x, float y, float z)
{
	this->scal = scale(mat4(), vec3(x, y, z));
}

mat4 visibleObj::getScal()
{
	return this->scal;
}

vec3 visibleObj::getScalXYZ()
{
	return vec3(scal[0].x, scal[1].y, scal[2].z);
}
